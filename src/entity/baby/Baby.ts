...
export class Baby extends BaseEntity {
    ...
    @Column({ comment: 'Peso do bebê', name: 'peso', type: 'float' })
    weight: number;

    @Column({ comment: 'Altura do bebê', name: 'altura', type: 'float' })
    height: number;

    @Column({ comment: 'Circunferência da cabeça do bebê', name: 'circunferencia', type: 'float' })
    ...
    @JoinTable({
        inverseJoinColumn: { name: 'fk_responsavel' },
        joinColumn: { name: 'fk_bebe' },
        name: 'bebe_responsavel',
    })
